...
{
private:
	/*std::pair<uint16_t, uint16_t> leftSide;
	std::pair<uint16_t, uint16_t> upSide;
	std::pair<uint16_t, uint16_t> rightSide;
	std::pair<uint16_t, uint16_t> downSide;*/

	/*const uint16_t xPos;
	const uint16_t yPos;

public:
	Sprite(uint16_t x, uint16_t y) : xPos(x), yPos(y) {};	
	uint16_t getX() { return xPos; }
	uint16_t getY() { return yPos; }*/
};
